import { RoleTag } from './shared.js';

const { useState, useEffect } = React;

export const AdminHeader = ({ onViewModeChange }) => (
    <header>
        <h1><i className="fas fa-users-cog"></i> Membership Admin Panel</h1>
        <div className="view-as-buttons">
            <button className="btn btn-secondary view-toggle-btn" onClick={() => onViewModeChange('member')}>
                <i className="fas fa-user-check"></i> View as Member
            </button>
            <button className="btn btn-secondary view-toggle-btn" onClick={() => onViewModeChange('unpaid')}>
