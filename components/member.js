...
import { StatusBadge, RoleTag } from './shared.js';

export const MembershipPromptSection = ({ settings }) => {
    const handleBecomeMember = async () => {
        try {
            const result = await MembershipService.postMembershipComment(settings.price);
            if (result.error) {
                console.error("Could not open comment dialog:", result.error);
            }
        } catch (error) {
            console.error("Error posting membership comment:", error);
        }
    };
...
export const MemberDashboard = ({ member, settings }) => {
    ...
    const handleExtendMembership = async () => {
        try {
            const result = await MembershipService.postExtendComment(settings.price);
            if (result.error) {
                console.error("Could not open comment dialog:", result.error);
            }
        } catch (error) {
            console.error("Error posting extend comment:", error);
        }
    };
